...
export default class ContractActions extends AbiActionForm {
  ...
  executeAction = async actionName => {
    ...
    try {
      const res = await this.props.contract.execute(actionName, parameters, {
        from: signer,
        value,
        ...options,
      })
      this.setState({ executing: false, actionResult: JSON.stringify(res.contractResult, null, 2) })

      // await queue.add(
      //   () => async () => {return {mined: true}},
      //   {

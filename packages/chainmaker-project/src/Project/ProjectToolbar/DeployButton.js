...
export default class DeployerButton extends PureComponent {
  ...
  updateContractFromPath = async contractPara => {
    const contract = await this.props.projectManager.readFile(contractPara.path)
    const abi = await this.props.projectManager.readFile(contractPara.abi)
    
    const contractObj = {
      contractName: contractPara.pathInProject,
      path: contractPara.path,
      contract,
      abi:JSON.parse(abi),
    }
    ...
    return this.setState({
      ...
      contractObj,
      constructorAbi: this.getConstructorAbi(contractObj.abi),
    })
  }
  ...
  needEstimate = () => {
    if (this.props.skipEstimate || true) {
      return false
    }

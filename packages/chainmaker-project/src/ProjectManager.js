...
function makeProjectManager(Base) {
  return class ExtendedProjectManager extends Base {
    ...
    async deploy(contractFileNode) {
      ...
      let contracts = await this.getBuiltContracts()
    ...
    async getBuiltContracts() {
      // for solidity
      return [{
        path: this.pathForProjectFile('Token.bin'),
        pathInProject: 'Token.bin',
        abi: this.pathForProjectFile('Token.abi'),
      }]
      // const settings = await this.checkSettings()
    ...
    async pushDeployment(contractObj, allParameters) {
      ...
      const { amount, parameters, ...override } = allParameters
      // const codeHash = networkManager.sdk.utils.sign.sha3(deploy.deployedBytecode)

      let signer = JSON.parse(await keypairManager.getSecret(allParameters.signer))

      const contractName = 'c' + new Date().getTime()
      let result = await networkManager.sdk.client.invokeClassCall('userContractMgr', "createUserContract",{
        contractName,
        contractVersion: '1.0.0',
        runtimeType: 'EVM',
        contractFilePath: contractObj.path,
        params: parameters.obj,
        userInfoList: [{
          signKey: signer.signKey,
          signCrt: signer.signCrt,
        }]
      })
      ...
      this.deployButton.setState({ pending: false })
      this.deployButton.closeModal()
      notification.success(t('contract.deploy.success'))

      contractObj.abi = JSON.stringify(contractObj.abi)
      redux.dispatch('ABI_ADD', {
        name: contractName,
        codeHash: result.txId,
        abi: contractObj.abi,
      })
      ...
      await this.saveFile(deployResultPath, JSON.stringify(Object.assign(result, {contractName}), null, 2))
    }
  }
}

...
export default class KeypairManagerModal extends PureComponent {
  static defaultProps = {
    ...
    warning: true,
    head: [t('abi.name'), 'fingerprint', t('explorer.page.balance')],
    actions: true,
    textActions: [t('keypair.import')],
    keypairText: 'Keypair',
    RevealSecretModal,
    ImportKeypairModal,
  }
      ...
      <tr key={`key-${validAddress}`} className='hover-flex'>
        ...
        <td>
          <div className='d-flex align-items-center'>
            <code className='small'>{keypair?.address?.substr(0, 38)}...</code>
